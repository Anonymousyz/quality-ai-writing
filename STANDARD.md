...
每维详情见 `principles/`。结构与修改顺序见 `notes/structure.md`、`notes/revision-order.md`。传统短摘见 `traditions/`。
...
1. 分清模式与写前三问。  
2. 先扫否决；触发即不合格。  
3. 按任务风险选深度：轻量通读 / 八维全检。  
4. 修改先大后小；事实与来源核验独立成门。  
5. 脱敏后经 case-intake 入 `cases/`；积累后修法。
